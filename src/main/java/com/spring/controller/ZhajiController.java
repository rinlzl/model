 ...
 * 闸机 */
@Controller
public class ZhajiController extends BaseController {

    @Autowired
    private ZhajiMapper dao;

    @Autowired
    private ZhajiService service;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/zhaji_list")
    public String list() {
        // 检测是否有登录，没登录则跳转到登录页面
        if (!checkLogin()) {
        ...
        Example example = new Example(Zhaji.class); //  创建一个扩展搜索类
        ...
        List<Zhaji> list = service.selectPageExample(example, page, pagesize); // 获取当前页的行数
    ...
    @RequestMapping("/zhaji_updt")
    public String updt() {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Zhaji mmm = service.find(id);
        assign("mmm", mmm);
        assign("updtself", 0);
     ...
     * @return
     */
    @RequestMapping("/zhajiinsert")
    public String insert() {
        _var = new LinkedHashMap(); // 重置数据
        String tmp = "";
        Zhaji post = new Zhaji(); // 创建实体类
        // 设置前台提交上来的数据到实体类中
        ...
        post.setXiangxi(Request.get("xiangxi"));
     ...
     * @return
     */
    @RequestMapping("/zhajiupdate")
    public String update() {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Zhaji post = new Zhaji();
        // 将前台表单数据填充到实体类
        ...
        if (!Request.get("xiangxi").equals("")) post.setXiangxi(Request.get("xiangxi"));
     ...
     *  后台详情
     */
    @RequestMapping("/zhaji_detail")
    public String detail() {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Zhaji map = service.find(id); // 根据前台url 参数中的id获取行数据
    ...
    public String delete() {
        ...
        Map map = Query.make("zhaji").find(id);
